...
//            }
    ...
    for (var j = 0; j < cloneRepos.length; j++) {
        var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/essential-studio/ej2-${cloneRepos[j]}-react-docs`;
        console.log('Clone has been started...!');
        var clone = shelljs.exec('git clone ' + gitPath + ' -b master' + ' ' + `./gitlapRepo/ej2-${cloneRepos[j]}-react-docs`, {
            silent: false
        });
        if (clone.code !== 0) {
            ...
            shelljs.cp('-rf', `./src/${cloneRepos[j]}/*`, `./gitlapRepo/ej2-${cloneRepos[j]}-react-docs/src`);
            shelljs.cd(`./gitlapRepo/ej2-${cloneRepos[j]}-react-docs`);
